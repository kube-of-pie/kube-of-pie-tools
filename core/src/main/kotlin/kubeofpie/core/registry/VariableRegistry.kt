 ...
 * injection and exposed by its dotted key. Writes are validated (writable flag plus
 * [Variable.allowedValues] membership) before being delegated to the variable.
 */
@Singleton
class VariableRegistry(private val variables: List<Variable>) {
    private val byKey: Map<String, Variable> = variables.associateBy(Variable::key)

    /**
     * Variables in registration order, optionally filtered.
     *
    ...
    fun read(key: String): Variable? = byKey[key]
